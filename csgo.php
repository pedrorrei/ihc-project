...
<head>
    ...
    <title>ZeroCostGaming - Cs:Go</title>
</head>
<style>
  .margem30 {
      margin-top: 30px;
...
<div class="text" style="display: flex;justify-content: center;margin-top: 50px;margin-bottom: 50px;margin-left: 50px;margin-right: 50px">
  <p style="font-size:500%;font-family:Impact, Haettenschweiler, 'Arial Narrow Bold', sans-serif;">CS:GO</p>
</div>
...
<div class="container" style="background-color: #1b0a39; border-radius: 5px;"> 
  <div class="containerjogo" style="margin-right: 10%;">
    <div class="image" style="display: flex;justify-content: center;">
      <img src="imagens/jogos/csgo.png" width="80%" class="rounded-corners"style="margin-top: 10%;margin-bottom: 10%;max-width: 300px;"></img>
    </div>
    <div class="text" style="margin-top: 20px;margin-bottom: 20px;">
    <p style="font-size:80%;margin-bottom: 5%;"><i>Counter-Strike: Global Offensive (CS:GO) é um jogo online desenvolvido pela Valve Corporation e pela Hidden Path Entertainment, sendo uma sequência de Counter-Strike: Source. É o quarto título principal da franquia.</i></p>
      <div>
      <?php if (isset($_SESSION['username'])): ?>
    <button id="wishlist-button-csgo" onclick="addToWishlist('<?php echo $_SESSION['username']; ?>', 'csgo')" class="btn btn-outline-success me-2" type="button"><i class="far fa-heart"></i> Add to Wishlist</button>
<?php else: ?>
    ...
    xhr.onreadystatechange = function() {
        if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
            ...
            if (response.message === "Game is in the wishlist") {
                ...
                button.onclick = function() { removeFromWishlist('<?php echo $_SESSION['username']; ?>', 'csgo'); };
            } else {
                button.innerHTML = '<i class="far fa-heart"></i> Add to Wishlist';
                button.className = 'btn btn-outline-success me-2';
                button.onclick = function() { addToWishlist('<?php echo $_SESSION['username']; ?>', 'csgo'); };
            }
        }
    }
    
    xhr.send("username=" + encodeURIComponent('<?php echo $_SESSION['username']; ?>') + "&game=csgo");
});
